...
void ST7735_Drawaxes(uint16_t axisColor, uint16_t bgColor, char *xLabel,char *yLabel1, uint16_t label1Color, char *yLabel2, uint16_t label2Color,int32_t ymax, int32_t ymin)
  ...
  ST7735_DrawFastHLine(10, 117, 60, axisColor);   // zaman 
  ST7735_DrawFastVLine(10, 17, 100, axisColor);			// sicaklik
  for(i=10; i<=110; i=i+10){													// X NOKTALAR
    ST7735_DrawPixel(i, 119, axisColor);
  }
  for(i=17; i<120; i=i+10){														// y noktalar
    ST7735_DrawPixel(9, i, axisColor);
  }
  i = 50;
  while((*xLabel) && (i < 100)){																// zaman yazisi
    ST7735_DrawCharS(i, 122, *xLabel, axisColor, bgColor, 1);
    i = i + 5;																									//zaman aralik
    xLabel++;
  }
